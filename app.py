...
# =============================================================================
# NUOVE FUNZIONI: CLUSTERING GEOGRAFICO + 2-OPT + PERNOTTO INTELLIGENTE (V2.1)
# =============================================================================
...
ALGO_VERSION = "v2.2_greedy_ultra"



# =============================================================================
# NUOVE FUNZIONI: CLUSTERING GREEDY ULTRA-FAST + 2-OPT LITE + PERNOTTO V2.1
# =============================================================================

ALGO_VERSION = "v2.2_greedy_fast_v2"


def _tour_2opt(lats, lons, siglas, circuity_dict, speed_dict, start_lat, start_lon, return_home=True):
    """
    Tour NN + 2-Opt LITE.
    - 2-Opt SOLO se n >= 5 (tour piccoli non hanno incroci significativi)
    - max_iterations = 50 (invece di 100)
    - Early exit immediato
    """
    n = len(lats)
    if n == 0:
    ...
    # 2-Opt solo per tour significativi
    if n >= 5:
        def _tour_dist(tour_idx):
            if not tour_idx: return 0.0
            total = haversine_km(start_lon, start_lat, lons[tour_idx[0]], lats[tour_idx[0]]) * \
                    get_effective_circuity(siglas[tour_idx[0]], circuity_dict)
            for i in range(len(tour_idx) - 1):
                a, b = tour_idx[i], tour_idx[i+1]
                total += haversine_km(lons[a], lats[a], lons[b], lats[b]) * \
            ...
            if return_home:
                ...
                total += haversine_km(lons[tour_idx[-1]], lats[tour_idx[-1]], start_lon, start_lat) * avg_circ
            return total
    ...
    km_intermediate = sum(
        haversine_km(lons[tour[i]], lats[tour[i]], lons[tour[i+1]], lats[tour[i+1]]) * \
        get_effective_circuity(siglas[tour[i+1]], circuity_dict)
    ...
    avg_speed = np.mean([speed_dict.get(siglas[idx], 45) for idx in tour]) if tour else 45
...
def create_geographic_daily_clusters(df_customers, rep_home_lat, rep_home_lon, 
    ...
    """
    Clustering greedy ULTRA-FAST.
    - Precalcola distanze da casa una sola volta
    - Seed = cliente più lontano, poi aggiunge i più vicini al SEED (non al centroide)
    - NO apply(), NO centroidi ricalcolati
    """
    ...
    for idx, row in df.iterrows():
        ...
        for _ in range(freq):
            visits.append({
                ...
                'lon': row['longitudine'],
                'sigla': row['sigla'],
                'dist_home': haversine_km(row['longitudine'], row['latitudine'], 
    ...
    if n_visits <= max_stops_per_day:
        ...
        f_lat, f_lon, l_lat, l_lon = _tour_2opt(
            visits_df['lat'].values, visits_df['lon'].values, 
            visits_df['sigla'].values,
            circuity_dict, speed_dict, rep_home_lat, rep_home_lon, 
            return_home=True
        )
        return [{
            'sector': 0, 'n_visite': n_visits, 'km': km, 'ore_viaggio': ore,
            'km_andata': km_andata, 'km_ritorno': km_ritorno, 
            'km_tour': km - km_andata - km_ritorno,
            'first_lat': f_lat, 'first_lon': f_lon, 
    ...
    # --- CLUSTERING GREEDY ULTRA-FAST ---
    # Ordina per distanza da casa (decrescente)
    visits_df = visits_df.sort_values('dist_home', ascending=False).reset_index(drop=True)

    # Precalcola matrice distanze tra tutti i clienti (una sola volta!)
    coords = visits_df[['lat', 'lon']].values
    n = len(coords)
    # Matrice distanze: solo metà superiore (simmetrica)
    ...
    while not assigned.all():
        # Trova primo non assegnato (il più lontano da casa)
        ...
        # Aggiungi i più vicini al SEED (non ricalcolare centroide!)
        while len(cluster_indices) < max_stops_per_day:
            remaining = np.where(~assigned)[0]
            if len(remaining) == 0:
                break

            # Trova il più vicino a qualsiasi cliente nel cluster
            # (usa distanza minima a qualsiasi membro del cluster)
...
def optimize_pernotto_v2(giornate, home_lat, home_lon, soglia_min, 
    ...
    """
    Pernottamento con logica anti-schizofrenia parametrizzabile.
    """
    n = len(giornate)
    if n < 2:
        return 0.0, 0.0, []
    ...
    # Calcola hotel e distanza da casa per ogni giornata
    for g in giornate:
        ...
        g['dist_da_casa'] = haversine_km(g['hotel_lon'], g['hotel_lat'], 
                                          home_lon, home_lat)

    # --- ORDINA GIORNATE PER DISTANZA DA CASA (crescente) ---
    # Questo migliora la coerenza del pernottamento: le giornate più lontane
    # sono consecutive nel vettore, facilitando coppie logiche
    ...
    for i in range(n - 1):
        ...
        circ_h = get_effective_circuity(g_j['sigle'][0] if g_j['sigle'] else 'RM', 
                                        circuity_dict)
        ...
        # Anti-schizofrenia: blocca se il giorno dopo è troppo vicino a casa
        troppo_vicino = g_j['dist_da_casa'] < (anti_schizofrenia_factor * soglia_km)

        if risparmio > soglia_km and not troppo_vicino:
            coppie.append((i, risparmio, dist_to_hotel, costo_attuale, 
                          costo_nuovo, troppo_vicino))
    ...
    for i, risp, dist_h, costo_att, costo_nuovo, troppo_vicino in coppie:
        ...
        if max_notti_consecutive >= 2 and (i + 2) < n and not used[i + 2]:
            ...
            circ2 = get_effective_circuity(g_i2['sigle'][0] if g_i2['sigle'] else 'RM', 
                                          circuity_dict)
            ...
            if risp_tot > 1.5 * soglia_km * 2 and not troppo_vicino_g2 and \
               notti_usate + 2 <= max_notti_anno:
    ...
    for start, end, risp_tot, n_notti in missioni:
        for idx in range(start, end):
            ...
            circ = get_effective_circuity(g_next['sigle'][0] if g_next['sigle'] else 'RM', 
                                          circuity_dict)
            ...
            speed_curr = np.mean([speed_dict.get(s, 45) for s in g_curr['sigle']]) \
                        if g_curr['sigle'] else 45
            speed_next = np.mean([speed_dict.get(s, 45) for s in g_next['sigle']]) \
                        if g_next['sigle'] else 45

            delta_ore = (-g_curr['km_ritorno'] / speed_curr) + \
                       (dist_to_hotel / speed_curr) + \
                       (-g_next['km_andata'] / speed_next) + \
                       (hotel_offset_km / speed_next)
...
def calculate_travel_km_tours_v2(df_customers, rep_home_lat, rep_home_lon, 
                                 ...
                                 anti_schizofrenia_factor=0.75):
    """
    Versione 2.1: clustering K-Means + 2-Opt + pernottamento anti-schizofrenico.
    """
    if len(df_customers) == 0:
        return 0.0, 0.0, [], 0

    giornate = create_geographic_daily_clusters(
        df_customers, rep_home_lat, rep_home_lon, max_stops_per_day,
    ...
    if pernotto_attivo and len(giornate) >= 2:
        delta_km, delta_ore, dettaglio_pernotto = optimize_pernotto_v2(
            ...
            anti_schizofrenia_factor
        )
        total_km += delta_km
        total_ore += delta_ore

    n_pernotti = sum(1 for m in dettaglio_pernotto if m['notti_in_missione'] > 0) \
                if pernotto_attivo else 0
...
# @st.cache_data(ttl=3600, show_spinner=False)  # DISABILITATO PER TEST
