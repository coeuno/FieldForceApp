...
def create_geographic_daily_clusters(df_customers, rep_home_lat, rep_home_lon, max_stops_per_day, circuity_dict, speed_dict):
    ...
    for idx, row in df.iterrows():
        ...
        for _ in range(freq):
            visits.append({
                'cliente_idx': idx, 'lat': row['latitudine'], 'lon': row['longitudine'],
                'sigla': row['sigla'], 'dist': haversine_km(row['longitudine'], row['latitudine'], rep_home_lon, rep_home_lat)
            })
    ...
    for cid in sorted(visits_df['cluster'].unique()):
        ...
        if len(cluster_df) > max_stops_per_day:
            ...
            cluster_df['sub_cluster'] = sub_kmeans.fit_predict(cluster_df[['lat', 'lon']].values)
            ...
            for sid in sorted(cluster_df['sub_cluster'].unique()):
                ...
                km, ore, end_lat, end_lon, km_andata, km_ritorno, f_lat, f_lon, l_lat, l_lon = _tour_2opt(
                    sub_df['lat'].values, sub_df['lon'].values, sub_df['sigla'].values,
                    circuity_dict, speed_dict, rep_home_lat, rep_home_lon, return_home=True
                )
                giornate.append({
                    ...
                    'clienti': [{'lat': r['lat'], 'lon': r['lon'], 'sigla': r['sigla']} for _, r in sub_df.iterrows()]
                })
        else:
            km, ore, end_lat, end_lon, km_andata, km_ritorno, f_lat, f_lon, l_lat, l_lon = _tour_2opt(
...
def optimize_pernotto_v2(giornate, home_lat, home_lon, soglia_min, max_notti_week, max_notti_consecutive, hotel_offset_km, gg_lavoro, circuity_dict, speed_dict):
    ...
    for i in range(n - 1):
        ...
        # LOGICA ANTI-SCHIZOFRENICA: Blocca se il giorno dopo è troppo vicino a casa
...
def main():
    ...
    if st.session_state.get('pending_removal'):
        ...
        n_a, n_b, n_c, n_d = int((orphan_df['classe'] == 'A').sum()), int((orphan_df['classe'] == 'B').sum()), int((orphan_df['classe'] == 'C').sum()), int((orphan_df['classe'] == 'D').sum())
        ...
        if calc_pressed and selected_receivers:
            with st.spinner("Calcolo scenario in corso... (può richiedere qualche secondo)"):
                ...
                # NOTA: Qui viene chiamata la funzione con il nuovo algoritmo v2
        ...
        if not selected_receivers:
            ...
            for r in selected_receivers:
                ...
                if n_ass > 0:
                    ...
                    sub_assigned = orphan_df.loc[assigned_indices]
                    na, nb, nc, nd = int((sub_assigned['classe'] == 'A').sum()), int((sub_assigned['classe'] == 'B').sum()), int((sub_assigned['classe'] == 'C').sum()), int((sub_assigned['classe'] == 'D').sum())
    ...
    for rep in active_reps:
        ...
        if len(sub) >= 3:
            ...
            if lon_h is not None:
                ...
                lon_h, lat_h = compute_hull_coords(sub)
                if lon_h is not None:
                    fig.add_trace(go.Scattermapbox(mode='lines', lon=lon_h, lat=lat_h, line=dict(width=1.5, color=rep_colors[rep]), fill='toself', fillcolor=rep_colors[rep], opacity=0.10, name=f"Zona {rep}", hoverinfo='name'))
